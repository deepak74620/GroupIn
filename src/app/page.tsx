...
type GroupWithMembers = RouterOutput["group"]["getAll"][number];
      ...
      <div className="container mt-12 flex flex-col items-center justify-center gap-4 px-4 py-8">
        ...
        <div className="mt-8 w-full max-w-2xl">
          ...
          <div className="mt-4 flex flex-col gap-4">
            ...
            {!isLoading && groups?.length === 0 && <p>You are not a member of any groups yet.</p>}
                ...
                <div key={group.id} className="rounded-xl bg-white/10 p-4 flex justify-between items-center">
                  <Link href={`/groups/${group.id}`} className="flex-grow hover:opacity-80 transition-opacity">
                    <div>
                      ...
                      <p className="text-sm text-gray-400">Created by: {group.createdBy.name}</p>
                      <p className="text-sm mt-2">
                        Members ({group.members.length}): {group.members.map(m => m.name).join(', ')}
                      </p>
                    </div>
                  </Link>
